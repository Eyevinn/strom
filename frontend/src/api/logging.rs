...
use strom_types::api::{
    GstLogLevelResponse, LogLevelResponse, SetGstLogLevelRequest, SetLogLevelRequest,
};
...
impl ApiClient {
    ...
    /// Get the current GStreamer debug level from the backend.
    pub async fn get_gst_log_level(&self) -> ApiResult<GstLogLevelResponse> {
        let url = format!("{}/gst-log-level", self.base_url);
        let response = self
            .with_auth(self.client.get(&url))
            .send()
            .await
    ...
    pub async fn set_gst_log_level(&self, filter: &str) -> ApiResult<GstLogLevelResponse> {
        let url = format!("{}/gst-log-level", self.base_url);
        let req = SetGstLogLevelRequest {
            filter: filter.to_string(),
        };
        let response = self
            .with_auth(self.client.put(&url).json(&req))
