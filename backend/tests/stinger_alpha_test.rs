...
/// GPU path — `glvideomixerelement`. Skips where no GL context can be created,
/// and fails instead under `STROM_REQUIRE_GL`, which CI sets on the one runner
/// that renders. Same rule as `vision_mixer_fx_test`.
#[test]
...
/// Skip unless GL works, except where CI says this platform must render.
///
/// A headless Linux runner cannot create a context, so skipping is the normal
/// path there. Without `STROM_REQUIRE_GL` the same silent skip would also hide a
/// GL alpha regression on the macOS runner, which can render.
fn gl_available_or_required() -> bool {
    ...
    assert!(
        ...
        "STROM_REQUIRE_GL is set but no GL context could be created — this platform \
         is supposed to render, so a skip here would hide a GL alpha regression"
    );
