...
fn fanout_cpu_per_consumer() {
    ...
    // A consumer that connected and then received nothing costs the server
    // almost nothing, which would read as a cheap viewer rather than a failed
    // run.
    assert!(
        received.iter().all(|&n| n > 0),
        "a consumer received no RTP buffers ({:?}), so the per-consumer figure is not a measurement",
        received
    );
}
