...
///   Used when the pipeline uses direct media timing (base_time=0, start_time=NONE),
///   which makes RTP timestamps reflect the reference clock directly. Applies to
///   PTP, NTP, TAI and Realtime clock types.
/// - `sender`: Media clock is asynchronously generated by the sender. Used for
///   Monotonic where the base_time is managed by GStreamer.
pub fn generate_mediaclk(flow_properties: Option<&FlowProperties>) -> String {
    ...
    if props.clock_type.uses_direct_media_timing() {
...
mod tests {
    ...
    fn test_mediaclk_realtime() {
        // Realtime uses direct media timing (base_time=0, start_time=NONE),
        // so RTP timestamps reflect the UTC wall clock directly.
        let props = FlowProperties {
