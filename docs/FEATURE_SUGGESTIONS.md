...
- **Publish straight to the big live platforms** — take Strom's RTMP output to YouTube Live
  and Twitch and write down what an operator has to set. The transport side arrives with the
  RTMP Output block (#771), still open: that sink speaks `rtmp` and `rtmps` and keeps the
  query string Twitch's `?bandwidthtest=true` rides on, so once it lands a test broadcast can
  be verified in Twitch Inspector without going live. The encoder side needs a look — the
  Video Encoder block pins no profile by default, which is fine from a 4:2:0 source but
